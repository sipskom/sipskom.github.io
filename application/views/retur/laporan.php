...
<div class="content-header">
    ...
    <div class="content">
        <div class="card card-indigo card-outline">
            ...
            <button target="_blank" onclick="window.open('<?=site_url('laporan/retur/by_full')?>','_blank')" name="cetak" id="cetak" style="display: block; margin-top: 10px ; margin-left: 35px" class="btn btn-flat btn-success"><i class="fa fa-print"></i> Cetak</button>    
            </div>
            <div id="filtgl" style="display: none; margin-top: 10px">
                <form method="POST" action="<?= site_url('laporan/retur/by_tgl') ?>" target="_blank">
            ...
            <div id="filbln" style="display: none; margin-top: 10px">
                <form method="POST" action="<?= site_url('laporan/retur/by_bln') ?>" target="_blank">
            ...
            <div id="filthn" style="display: none; margin-top: 10px">
                <form method="POST" action="<?= site_url('laporan/retur/by_thn') ?>" target="_blank">
          ...
          <?php foreach ($result as $key => $data) { ?>
						<tr>
							...
							<td class="text-center"><?= date('m/d/Y', strtotime($data->tgl_retur)) ?></td>
							<td><?= $data->no_retur ?></td>
							<td><?= $data->id_customer == 0 ? 'Umum' : $data->nama_customer ?></td>
              <td><?= $data->no_fakt ?></td>
              <td><?= $data->nama_barang ?></td>
              <td class="text-right"><?= $data->qty_jual ?></td>
              <td class="text-right"><?= rp($data->harga_jual) ?></td>
							<td class="text-right"><?= $data->qty_retur ?></td>
              <td class="text-right"><?= $data->denda ?>%</td>
              <td class="text-right"><?= rp($data->total_retur) ?></td>
              <td>
              <button target="_blank" onclick="window.open('<?=site_url('retur/cetak/'.$data->id_retur)?>','_blank')" class="btn btn-xs btn-flat btn-secondary"> Cetak Nota</button>  
              </td>
						</tr>
					<?php } ?>
				</tbody>
				<tfoot>
					<tr>
                    <td colspan="11" class="text-right" style="color: blue;">
