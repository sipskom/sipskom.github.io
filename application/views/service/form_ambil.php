    ...
    <div class="content">
        <div class="card card-<?=$warna?> card-outline">
            <div class="card-header">
                <h3 class="card-title"><?=$page?> Service</h3>
                        ...
                        <form action="<?=site_url('service/proses') ?>" method="post">
                            ...
                            <div class="row">
                                <div class="form-group">
                                    ...
                                    <textarea name="kerusakan" id="kerusakan" cols="23" rows="2" class="form-control" readonly><?=$row->kerusakan?></textarea>
                                </div>
                                <div class="form-group" style="margin-left: 50px;">
                                    <label for=""> Sparepart </label>
                                    <textarea name="sparepart" id="sparepart" cols="23" rows="2" class="form-control" readonly><?=$row->sparepart?></textarea>
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group">
                                    <label for=""> Biaya Service </label>
                                    <input type="number" value="<?=$row->biaya_service?>" name="biaya_service" id="biaya_service" class="form-control" readonly> 
                                </div>
                                <div class="form-group" style="margin-left: 50px;">
                            ...
                            <div class="row">
                                <div class="form-group">
                                    ...
                                    <input type="number" value="" name="dibayarkan" id="dibayarkan" class="form-control" required> 
                                </div>
                                <div class="form-group" style="margin-left: 50px;">
                                    <label for=""> Kembalian </label>
                                    <input type="number" value="" name="kembalian" id="kembalian" class="form-control" readonly> 
                                </div>
                            </div>
                            ...
                            <div class="form-group float-right">
                                ...
                                <button type="submit" name="<?=$page?>" class="btn btn-info" onclick="window.open('<?=site_url('service/ambil_print/'.$row->id_service)?>','_blank')">
...
<script>
    function count_edit_modal() {
		var biaya = $('#biaya_service').val();
		var dp = $('#dp').val();
        var dibayarkan = $('#dibayarkan').val();

        sisa = biaya - dp;
        total = dibayarkan - sisa;
		$('#kembalian').val(total);

	};

	$(document).on('keyup mouseup', '#dibayarkan', function() {
		count_edit_modal();
	});
</script>